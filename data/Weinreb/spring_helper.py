...
def preprocess_and_pca(E, total_counts_normalize=True, norm_exclude_abundant_gene_frac=1, min_counts=3, min_cells=5,
                       min_vscore_pctl=85, gene_filter=None, num_pc=50, sparse_pca=False, show_vscore_plot=False):
    '''
    ...
    if gene_filter is None:
        ...
        gene_filter = filter_genes(E, min_vscore_pctl=min_vscore_pctl, min_counts=min_counts, min_cells=min_cells,
...
def get_force_layout(links, n_cells, n_iter=100, edgeWeightInfluence=1, barnesHutTheta=2, scalingRatio=1, gravity=0.05,
...
# def make_spring_subplot(E, gene_list, save_path, base_ix=None, normalize=True, exclude_dominant_frac=1.0, min_counts=3,
#                         min_cells=5, min_vscore_pctl=75, show_vscore_plot=False, exclude_gene_names=None, num_pc=30,
#                         sparse_pca=False, pca_norm=True, k_neigh=4, cell_groupings={}, num_force_iter=100,
#                         output_spring=True, precomputed_pca=None, gene_filter=None, custom_colors={},
#                         exclude_corr_genes_list=None, exclude_corr_genes_minCorr=0.2, dist_metric='euclidean',
#                         use_approxnn=False, run_doub_detector=False, dd_k=50, dd_frac=5, dd_approx=True,
...
#                 gene_filter = filter_genes(E, base_ix, min_vscore_pctl=min_vscore_pctl, min_counts=min_counts,
#                                            min_cells=min_cells, show_vscore_plot=show_vscore_plot)
...
#                 gene_filter = remove_corr_genes(E, gene_list, exclude_corr_genes_list, gene_filter,
...
#                 keep_ix = np.array(
#                     [ii for ii, gix in enumerate(gene_filter) if gene_list[gix] not in exclude_gene_names])
...
#         doub_score, doub_score_full, doub_labels = woublet.detect_doublets([], counts=tot_counts_final,
#                                                                            doub_frac=dd_frac, k=dd_k,
#                                                                            use_approxnn=dd_approx, precomputed_pca=Epca)
...
#         np.savez_compressed(save_path + '/intermediates.npz', Epca=Epca, gene_filter=gene_filter,
...
def plot_groups(x, y, groups, lim_buffer=50, saving=False, fig_dir='./', fig_name='fig', res=300, close_after=False,
                title_size=12, point_size=3, ncol=5):
    import matplotlib.pyplot as plt
...
def rank_enriched_genes(E, gene_list, cell_mask, min_counts=3, min_cells=3):
    ...
    print
    '%i cells in group' % (sum(cell_mask))
    print
    'Considering %i genes' % (sum(gix))
